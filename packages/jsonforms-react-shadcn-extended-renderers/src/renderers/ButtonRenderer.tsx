...
import { useShadcnComponents } from '@chobantonov/jsonforms-react-shadcn-renderers';
...
export const shadcnButtonRendererTester: RankedTester = rankWith(1, uiTypeIs('Button'));

export const ShadcnButtonRenderer = ({ enabled, visible, uischema }: LayoutProps & { uischema: ShadcnButtonElement }) => {
  ...
  const label = options.label ?? options.text ?? uischema.label ?? uischema.text ?? uischema.name ?? 'Action';
...
export const ShadcnButtonRendererWithProps = withJsonFormsLayoutProps(ShadcnButtonRenderer);
