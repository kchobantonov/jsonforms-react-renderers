...
export const ShadcnAgGridArrayControl = (props: ControlProps) => {
  ...
  const itemSchema = (Array.isArray(props.schema.items) ? props.schema.items[0] : props.schema.items) as JsonSchema | undefined;
  ...
  const columns = React.useMemo<ColDef[]>(
    () => Object.entries(properties).map(([field, schema]) => ({
      ...
      filter: true,
      resizable: true,
      cellDataType: schema.type === 'number' || schema.type === 'integer' ? 'number' : schema.type === 'boolean' ? 'boolean' : 'text',
    })),
  ...
  return (
    <InputShell id={id} label={props.label} required={props.required} description={props.description} errors={props.errors}>
      <div className='shadcn-jsonforms-grid-actions'>
        <Button size='sm' disabled={!props.enabled} onClick={() => props.handleChange(props.path, [...rows, defaultValue(itemSchema ?? {})])}>Add row</Button>
        <Button variant='destructive' size='sm' disabled={!props.enabled || selected.length === 0} onClick={() => {
          const removals = new Set(selected);
          props.handleChange(props.path, rows.filter((_row, index) => !removals.has(index)));
          setSelected([]);
        }}>Remove selected</Button>
      </div>
      <div id={id} className='shadcn-jsonforms-ag-grid' style={{ height: props.uischema.options?.height ?? 400 }}>
        <AgGridReact
          theme={themeQuartz}
          rowData={rows.map((value, index) => ({ ...value, __jsonformsIndex: index }))}
          columnDefs={columns}
          defaultColDef={{ flex: 1, minWidth: 120 }}
          rowSelection={{ mode: 'multiRow' }}
          getRowId={(params) => String(params.data.__jsonformsIndex)}
          onSelectionChanged={(event) => setSelected(event.api.getSelectedRows().map((row) => row.__jsonformsIndex))}
...
export const AgGridArrayControlRenderer = withJsonFormsControlProps(ShadcnAgGridArrayControl);
