import { ControlProps, JsonSchema, RankedTester, and, rankWith, schemaMatches, uiTypeIs } from '@jsonforms/core';
...
export const nullControlTester: RankedTester = rankWith(
  2,
  and(uiTypeIs('Control'), schemaMatches((schema: JsonSchema) => schema.type === 'null'))
);
    ...
    <InputShell id={id} description={props.description} errors={props.errors}>
      <label className='shadcn-jsonforms-checkbox-label' htmlFor={id}>
        <Checkbox
          ...
          onCheckedChange={(checked) => props.handleChange(props.path, checked ? null : undefined)}
        />
