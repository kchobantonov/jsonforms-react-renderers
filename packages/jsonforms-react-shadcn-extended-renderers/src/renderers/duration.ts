...
export const parseDuration = (value: unknown): DurationParts | null => {
  ...
  if (week?.groups)
    return { ...EMPTY_DURATION_PARTS, weeks: number(week.groups.weeks) };
  const match = value.trim().match(DURATION);
  if (
    !match?.groups ||
    !Object.values(match.groups).some((part) => part !== undefined)
  )
    return null;
  return {
    weeks: 0,
...
export const formatDurationIso = (input: DurationParts): string => {
  ...
  const date = `${parts.years ? `${parts.years}Y` : ''}${
    parts.months ? `${parts.months}M` : ''
  }${parts.days ? `${parts.days}D` : ''}`;
  const time = `${parts.hours ? `${parts.hours}H` : ''}${
